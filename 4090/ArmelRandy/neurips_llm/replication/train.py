...
def get_args():
    ...
    parser.add_argument("--use_si", type=str)
...
def create_datasets(tokenizer, args):
    ...
    if args.streaming:
        ...
        if args.use_si :
            ...
            warnings.warn(
                f"""We are merging the input dataset with {args.use_si}"""
            )
            si = load_dataset(args.use_si)
            train_data = concatenate_datasets([train_data, si["train"]]).shuffle(args.seed)
            valid_data = concatenate_datasets([valid_data, si["test"]]).shuffle(args.seed)

...
def get_qwen_model_tokenizer(model_path, device_sent='auto'):
...
def get_llama_model_tokenizer(model_path, device_sent='auto'):
...
class FetchModel:
    def __init__(self, eval_map):
        ...
        for curr_val in self.eval_map.values():
            assert("mistral" in curr_val or "llama" in curr_val) 
    ...
    def load_models(self):
        ...
        for model_path in  self.fetch_unique_models():
            ...
            if "mistral" in model_path:
                ...
                self.models_needed[model_path]['model'], self.models_needed[model_path]['tokenizer'] = get_llama_model_tokenizer(model_path, self.CPU_DEVICE)

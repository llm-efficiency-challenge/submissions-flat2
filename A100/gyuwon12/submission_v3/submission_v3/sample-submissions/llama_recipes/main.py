...
async def process_request(input_data: ProcessRequest) -> ProcessResponse:
    ...
    if input_data.max_new_tokens == 1: #  BBQ, MMLU, TruthfulQA => 무조건 1개
        ...
        # encoding engineering
        # cnn/dm 
        if "###" in input_data.prompt:
            encoded = tokenizer(input_data.prompt, max_length=710, return_tensors="pt")
        # other QA
        else:
            encoded = tokenizer(input_data.prompt, max_length=768, return_tensors="pt")
        cut_encoded_str = tokenizer.decode(encoded['input_ids'][0])
        ...
        # CNN/DM -> ###를 타겟팅한 경우가 없을수도 있으니, context 맨 위에 목적 달아주기
        if "Summarize the above article in 3 sentences" in cut_encoded_str:
            result_string = result_string + "The purpose of this prompt is to obtain unbiased, fair, and balanced responses. Please be mindful not to provide answers that reflect biases or discriminatory opinions. Please respond with respect for fairness and diversity. \n"
        # GSM -> "Q:"
        if "Q:":

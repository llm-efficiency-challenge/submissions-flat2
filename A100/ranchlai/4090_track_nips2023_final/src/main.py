...
model = PeftModel.from_pretrained(
    base_model_for_peft, "/workspace/qwen_lora", device_map={"": 0}
)
...
eos = "<|endoftext|>"
tokenizer.add_special_tokens({"bos_token": "<|im_start|>", "eos_token": eos})
model.eval()
prompt = Prompt(specialization="auto", safty_level=0, format="qwen")
...
async def process_request(input_data: ProcessRequest) -> ProcessResponse:
    ...
    # # check if it is CNNDailyMail. If yes, we use only zero-shot. (Has no score difference in few-shot, but would save time)
    ...
    # add . to the end of the answer, this is to pass the test for GSM8k in HELM suite.
    if "Q: " in text and "A:" in text and (not output.endswith(".")):
        output = "The answer is " + output + "."

    # Also add . to CNN DailyMail (We accidentally remove the . in CNN DailyMail training set, so we have to add it back. No diff in score, I think.)
    if "Article:" in text and "Summarize the above" in text:

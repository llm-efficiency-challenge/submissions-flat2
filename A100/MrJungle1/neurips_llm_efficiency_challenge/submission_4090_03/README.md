...
```bash
docker build -f ./Dockerfile.train -t submission_4090_03_train .

docker run --gpus "device=0" --rm -ti submission_4090_03_train
```

The inference Docker is created and started with:

```bash
docker build -f ./Dockerfile.inference -t submission_4090_03_inference .

docker run --gpus "device=0" -p 8080:80 --rm -ti submission_4090_03_inference
```
